...
### 12. Extrato de liquidação: relatório analítico via SQL nativo
- **Motivo**: rota de relatório cross-lote (`GET /api/v1/extrato-liquidacao`), com filtros combináveis por período, `ativo` (busca parcial) e moeda, e paginação — o tipo de consulta que tende a crescer em filtro/ordenação e onde controlar o SQL/plano de execução diretamente compensa mais que a conveniência do ORM. Decisão explícita do usuário de usar SQL nativo em vez de JPA/Criteria **só para este relatório**; o resto do sistema continua 100% Hibernate — é uma exceção pontual, não mudança de padrão geral.
- **Implementação**: `ExtratoLiquidacaoJdbcAdapter` (`adapter.out.persistence`) usa `NamedParameterJdbcTemplate` (autoconfigurado pelo Spring Boot via `spring-boot-starter-data-jpa`, nenhuma dependência nova) em vez de repositório JPA. Filtros opcionais via `(:param IS NULL OR ...)`. Contagem total de páginas obtida com `count(*) OVER()` na mesma consulta, evitando um round-trip extra só para o total.
- **`@SQLRestriction` não se aplica a SQL nativo**: `@SQLRestriction("deleted_at is null")` (ver item "Banco de dados") só filtra consultas que passam pelo Hibernate. Este adapter é o único no sistema que lê via `JdbcTemplate` puro, então o `WHERE r.deleted_at IS NULL` é manual e obrigatório na query — omiti-lo reintroduziria recebíveis soft-deletados no relatório.
- **Escopo**: só recebíveis com `status = 'LIQUIDADO'` aparecem — é o que "liquidação" significa no domínio (item 9: liquidação é sempre integral, um evento por recebível).
- **Índices dedicados** (migration `V202609181011`): índice parcial `(liquidado_em desc) where status = 'LIQUIDADO'` (cobre o filtro de período + ordenação, o caso de uso central do relatório) e índice GIN trigram (`pg_trgm`) em `ativo` para a busca parcial case-insensitive não degradar para `seq scan` com volume.

### 13. Teste de contrato da API (`openapi.yaml`)
- **Problema**: `openapi.yaml`, na raiz do repo, é um snapshot estático gerado manualmente (via `curl .../v3/api-docs.yaml`) para consumidores externos lerem sem precisar subir a aplicação. Sem verificação automatizada, esse arquivo diverge silenciosamente do comportamento real da API sempre que um endpoint muda — foi exatamente o que aconteceu com a feature de extrato de liquidação (item 12): o arquivo ficou desatualizado até este teste ser criado.
- **Solução**: `OpenApiContractTest` (`adapter.in.web`) sobe o contexto Spring completo, busca a spec real via `GET /v3/api-docs.yaml` (gerada em runtime pelo springdoc a partir das anotações do código) e compara, como estrutura (via SnakeYAML, já dependência transitiva do Spring Boot — nenhuma lib nova), com o arquivo commitado. Divergência de schema/endpoint/parâmetro falha o build com uma mensagem indicando como regenerar o arquivo.
- **Por que comparar estrutura e não texto bruto**: formatação/ordem de chaves do YAML não importa para quem consome a API; comparar como `Map` evita falsos positivos por diferença cosmética.
- **Exceção deliberada**: o campo `servers` é ignorado na comparação — o springdoc infere host/porta a partir da requisição HTTP usada para gerar a spec, e a requisição sintética do MockMvc (sem porta real) sempre diverge do `http://localhost:8080` de uma chamada real via `docker-compose`. Isso não faz parte do contrato (endpoints/schemas/parâmetros), só do ambiente onde a spec foi capturada.
